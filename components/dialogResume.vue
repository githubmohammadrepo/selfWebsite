          ...
          <v-row>
            <v-col v-for="item in 5" :key="item">
              <template>
                ...
                <v-card-title primary-title class="blue accent-4 text-center">
                  <subheader class="m-auto">سایت فروشگاهی مونیکا</subheader>
                </v-card-title>
                  <!-- <v-img
                        ...
                        >
                          ...
                          <v-list-item-content>
                            ...
                            <v-list-item-title v-if="item.subIcons">
                                <v-tooltip top  v-for="subIcon in item.subIcons" :key="subIcon">
                                <template v-slot:activator="{ on, attrs }">
                                  <v-icon v-bind="attrs" v-on="on" :color="subIcon.color" :title="subIcon.tooltop" v-text="subIcon.icon"></v-icon>
                                </template>
...
export default {
  ...
  computed: {
    dialog:{
      get(){
        return this.$store.state.resume.dialogResume;
      },
      set(value){
